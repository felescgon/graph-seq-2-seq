...
class StepByStep(object):
    def __init__(self, model, loss_fn, optimizer, save_checkpoints=False, checkpoints_directory='', checkpoint_context={}, initial_epoch=0, initial_losses=[], intial_val_losses=[], device=None):
        ...
        self.losses = initial_losses
        self.val_losses = intial_val_losses
        self.learning_rates = []
    ...
    def _make_train_step_fn(self):
        ...
        def perform_train_step_fn(x, y):
            ...
            G_loss_V1 = torch.mean(torch.abs(torch.sqrt(yhat.var(dim=0, unbiased=False) + 1e-6) - torch.sqrt(y.var(dim=0, unbiased=False) + 1e-6)))
            G_loss_V2 = torch.mean(torch.abs((yhat.mean(dim=0)) - (y.mean(dim=0))))
    ...
    def _make_val_step_fn(self):
        ...
        def perform_val_step_fn(x, y):
            ...
            G_loss_V1 = torch.mean(torch.abs(
                torch.sqrt(yhat.var(dim=0, unbiased=False) + 1e-6) - torch.sqrt(y.var(dim=0, unbiased=False) + 1e-6)))
            G_loss_V2 = torch.mean(torch.abs((yhat.mean(dim=0)) - (y.mean(dim=0))))
            G_loss_V = G_loss_V1 + G_loss_V2
            loss = loss + G_loss_V
    ...
    def _mini_batch(self, validation=False):
        ...
        for i, (x_batch, y_batch) in enumerate(mini_batch_progress_bar):
            ...
            mini_batch_progress_bar.set_description(
                f'Minibatch loss: {mini_batch_loss:.4f}'
            )
